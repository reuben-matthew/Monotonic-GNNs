...
def apply_model(cd_graph:CDGraph, device, model, trace_collector=None):
    ...
    # Apply model
    model.eval()
    final_output, intermediates = model(data)
    if trace_collector is not None:
        trace_collector.cd_graph = cd_graph
        trace_collector.activations = intermediates  # intermediates[i] is the output of layer i

    # PyTorch Decoding: pytorch geometric graph -> cd_graph
    return CDGraph(cd_graph.col_size, cd_graph.delta, final_output.detach().clone(), cd_graph.edges,

...
def apply_model(cd_graph:CDGraph, device, model, trace_collector=None):
    ...
    # Apply model
    model.eval()
    activations = model(data)  # note: middle layers comes already detached
    output_features = activations[-1].detach().clone()
    if trace_collector is not None:
        trace_collector.cd_graph = cd_graph
        trace_collector.activations = [a.detach().clone() for a in activations]

    # PyTorch Decoding: pytorch geometric graph -> cd_graph
    return CDGraph(cd_graph.col_size, cd_graph.delta, output_features, cd_graph.edges,
